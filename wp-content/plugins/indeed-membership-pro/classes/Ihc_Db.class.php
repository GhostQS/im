...
<h4>New Updates will be available!</h4>',
	...
	public static function get_order_data_by_id($order_id=0){
		 ...
		 */
		 ...
		 if ($order_id){
			 global $wpdb;
			 $table = $wpdb->prefix . 'ihc_orders';
			 $q = $wpdb->prepare("SELECT * FROM $table WHERE id=%d", $order_id);
			 $data = $wpdb->get_row($q);
			 if ($data){
			 	$array = (array)$data;
	...
	public static function get_taxes_by_country($country='', $state=''){
		...
		global $wpdb;
		$table = $wpdb->prefix . 'ihc_taxes';
		$q = $wpdb->prepare("SELECT * FROM $table WHERE country_code=%s", $country);
		if ($state){
			$q .= " AND state_code=%s ";
			$q = $wpdb->prepare($q, $state);
			$data = $wpdb->get_results($q);
			if (empty($data)){
				$q = $wpdb->prepare("SELECT * FROM $table WHERE country_code=%s AND state_code='' ", $country);
				$data = $wpdb->get_results($q);
			}
		} else {
			$q .= " AND state_code='' ";
	...
	public static function get_user_levels($uid=0, $check_expire=FALSE){
		 ...
		 */
		 ...
		 if ($uid){
			 ...
			 $levels = get_option('ihc_levels');
			 $table = $wpdb->prefix . "ihc_user_levels";
			 $q = $wpdb->prepare("SELECT * FROM $table WHERE user_id=%d", $uid);
			 $data = $wpdb->get_results($q);
			 if ($data){
			 	foreach ($data as $object){
			 		$temp = (array)$object;
	...
	public static function get_user_level_data($uid=0, $lid=0){
		global $wpdb;
		$table = $wpdb->prefix . 'ihc_user_levels';
		$q = $wpdb->prepare("SELECT start_time, expire_time, notification, status FROM $table WHERE user_id=%d AND level_id=%d", $uid, $lid);
		$data = $wpdb->get_row($q);
		return (array)$data;
	}
		 ...
		 if ($data){
			...
			if (empty($data['how_many_codes'])){
				...
				if (!empty($data['code'])){
					...
					$q = $wpdb->prepare("SELECT * FROM $table WHERE code=%s;", $data['code']);
				 	$check = $wpdb->get_row($q);
	...
	public static function get_all_gift_codes($limit=30, $offset=0){
		 ...
		 */
		 ...
		 $q = $wpdb->prepare("SELECT * FROM $table WHERE status=2 ORDER BY id DESC LIMIT %d OFFSET %d ", $limit, $offset);
		 $data = $wpdb->get_results($q);
		 if ($data){
		 	foreach ($data as $object){
		 		$temp = unserialize($object->settings);
	...
	public static function is_order_id_for_uid($uid=0, $order_id=0){
		 ...
		 */
		 if ($uid && $order_id){
		 	 global $wpdb;
			 $table = $wpdb->prefix . 'ihc_orders';
			 $q = $wpdb->prepare("SELECT * FROM $table WHERE uid=%d AND id=%d;", $uid, $order_id);
			 $check = $wpdb->get_row($q);
			 if ($check && !empty($check->id)){
			 	return TRUE;
			 }
	...
	public static function transactions_get_total_for_user($uid=0){
		 ...
		 */
		 if ($uid){
		 	 global $wpdb;
			 $table = $wpdb->prefix . "indeed_members_payments";
			 $q = $wpdb->prepare("SELECT COUNT(*) as c FROM $table WHERE u_id=%d", $uid);
			 $data = $wpdb->get_row($q);
			 if ($data && !empty($data->c)){
			 	return $data->c;
			 }
	 ...
	 */
	 ...
	 public static function user_get_level_details($uid=0, $lid=-1){
		...
		if ($uid && $lid>-1){
		 	$table = $wpdb->prefix . 'ihc_user_levels';
			$q = $wpdb->prepare("SELECT start_time, update_time, expire_time FROM $table WHERE user_id=%d AND level_id=%d", $uid, $lid);
		 	$data = $wpdb->get_results($q);
		 	return (array)$data;
		}
		return $data;
	...
	public static function does_user_meta_exists($uid=0, $meta_key=''){
		...
		if ($uid && $meta_key){
			$table = $wpdb->prefix . 'usermeta';
			$q = $wpdb->prepare("SELECT umeta_id FROM $table WHERE meta_key=%s AND user_id=%d ", $meta_key, $uid);
			$data = $wpdb->get_row($q);
