...
<?php if ( ! comments_open() && get_comments_number() && post_type_supports( get_post_type(), 'comments' ) ) : ?>
	<p class="no-comments"><?php esc_html_e( 'Ideas are closed.', 'Avada' ); ?></p>
<?php endif; ?>
...
<?php if ( comments_open() ) : ?>
	...
	$comments_args = array(
		...
		'must_log_in'          => '<p class="must-log-in">' . sprintf( esc_html__( 'You must be %1$slogged in%2$s to post a idea.', 'Avada' ), '<a href="' . wp_login_url( apply_filters( 'the_permalink', get_permalink() ) ) . '">', '</a>' ) . '</p>',
		...
		'label_submit'         => esc_html__( 'Objavi ideju', 'Avada' ),
	);
